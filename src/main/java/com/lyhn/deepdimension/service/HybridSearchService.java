...
public class HybridSearchService {
    ...
    public List<SearchResult> searchWithPermission(String query, String userId, int topK) {
        ...
        try {
            ...
            // 生成查询向量，将用户查询转为ElasticSearch可用的格式（浮点数列表）
            ...
            SearchResponse<EsDocument> response = esClient.search(s -> {
                s.index("knowledge_base");//存储文档向量化数据的索引
                // KNN 召回，基于语义相似度召回大量相关文档，确保不遗漏语义相关但关键词不匹配的内容
                // 语义相似度匹配
                int recallK = topK * 30; // KNN 召回窗口，取 topK 的 30 倍
                s.knn(kn -> kn
                        .field("vector")
