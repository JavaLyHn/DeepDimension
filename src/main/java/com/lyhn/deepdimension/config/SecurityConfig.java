...
import com.lyhn.deepdimension.filter.JwtAuthenticationFilter;
import com.lyhn.deepdimension.filter.OrgTagAuthorizationFilter;
 ...
 *
 * 本项目RBAC实现流程:  1 首先用户通过注册界面输入账号和密码，系统会检查用户是否存在，如果通过校验，回味该用户分配一个默认角色USER
 *                    2 注册完成后，用户通过登录接口进行登录，系统验证通过以后，会生成一个包含用户信息的jwt token，返回给客户端
 *                    3 客户端后续在访问其他接口时，需要将这个token放在请求头的Authorization字段中，格式为Bearer token，系统会通过自定义的JwtAuthenticationFilter
 *                      进行验证，验证通过后，用户就可以访问该接口了，并将用户信息放到SpringSecurity上下文中
 *                    4 后续的接口请求中，系统会从上下文中获取用户信息，根据SpringSecurity的配置，进行权限校验，确保用户只能访问其被授权的资源
 */
